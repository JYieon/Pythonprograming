...
while not exit:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if len(key_name) == 1:
                if (ord(key_name) >= 65 and ord(key_name) <= 90) or (ord(key_name) >= 97 and ord(key_name) <= 122): #소문자나 대문자 걸러내는 코드
                    entry_text = key_name.upper()
                else : entry_text = ""
            else : entry_text = ""
            print(key_name)
    #사용자가 key를 입력할 때마다 화면에 출력됨
