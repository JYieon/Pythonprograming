...
while not exit:
    ...
    while not exit:
        ...
        if pygame.time.get_ticks() % 1000 > 500: #1초마다 문자 깜빡
            screen.blit(guide, guide_pos) #가이드 출력
        pygame.display.flip()
    ...
    while not exit:
        ...
        if drop == True:
            ...
            G = (F[0], F[1]+r_head)
        if try_num >= 1 or save == True:
            pygame.draw.circle(screen, white, G, r_head, 3)
        H = (G[0], G[1]+r_head)
        I = (H[0], H[1]+r_head)
        if try_num >= 2 or save == True:
        ...
        J = tup_r(J)
        K = tup_r(K)
        if try_num >= 3 or save == True:
            pygame.draw.line(screen, white, I, J, 3)
        if try_num >= 4 or save == True:
            pygame.draw.line(screen, white, I, K, 3)
        L = (I[0], I[1]+l_arm)
        if try_num >= 5 or save == True:
        ...
        M = tup_r(M)
        N = tup_r(N)
        if try_num >= 6 or save == True:
            pygame.draw.line(screen, white, L, M, 3)
        if try_num >= 7 or save == True:
        ...
        entry_bg_size = 80
        pygame.draw.rect(screen, white, tup_r((size[0]/2-entry_bg_size/2, size[1]*17/18-entry_bg_size/2, entry_bg_size, entry_bg_size)))
