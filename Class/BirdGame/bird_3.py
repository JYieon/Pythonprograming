...
while not exit:
    ...
    if left_go == True and right_go == False:
        ...
        player.timer += player.ani_move # ani_move만큼 timer 누적
        player.img = p_list[int(player.timer)%len(p_list)]
        player.img = pygame.transform.flip(player.img, True, False) #flip() : 주어진 그림 좌우반전
        if player.pos[0] <= 0:
    ...
    else : player.img = person_static #정지
    # 4-4. 그리기
    screen.fill(white)
    player.show()
    # 4-5. 업데이트
