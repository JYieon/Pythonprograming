...
player = person()
#새, 새똥은 여러개이기 때문에 list로 설정하여 생성, 삭제 관리
bb_list = []
dd_list = []
...
while not exit:
    ...
    # 4-3. 입력, 시간에 따른 변화
    # 새 생성
    if random.random() < 0.05: #5%의 확률로 if문에 충족된다.
        bb = bird()
        bb_list.append(bb) #bb리스트에 추가
    # 플레이어 움직임
    ...
    # 새 움직임 
    for bb in bb_list: #bb_list에 있는 새를 꺼내서 관리
    ...
    # 새 소멸
    del_list = []
    for i, bb in enumerate(bb_list): #enumerate : index를 알아내면서 순회
        if bb.pos[0] < -bb.size[0] or bb.pos[0] > size[0]: #x축 좌표가 그림을 심하게 벗어나는 경우
            del_list.append(i)
    del_list.reverse() #리스트 뒤집기
    for i in del_list:
        del bb_list[i]  
    # 새똥 소멸
    del_list = []
