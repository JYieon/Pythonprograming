...
b_list = [[img_read("bird_0_0",b_r), img_read("bird_0_1",b_r),img_read("bird_0_2",b_r),img_read("bird_0_3",b_r),img_read("bird_0_2",b_r),img_read("bird_0_1",b_r)],
          ...
          [img_read("mybird1",b_r), img_read("mybird2",b_r),img_read("mybird3",b_r),img_read("mybird4",b_r),img_read("mybird3",b_r),img_read("mybird2",b_r)]]
dung_list = [img_read("dung", 0.15), img_read("poop", 0.15)]
...
point_font = pygame.font.Font("C:/Windows/Fonts/arial.ttf", 50)
finalp_font = pygame.font.Font("C:/Windows/Fonts/arial.ttf", 80)
ready_font = pygame.font.Font("C:/Windows/Fonts/arial.ttf", 80)
ready2_font = pygame.font.Font("C:/Windows/Fonts/arial.ttf", 20)
...
while not exit:
    ...
    while not exit:
        ...
        pygame.display.flip()    
    del bb #ready화면에 새 지우기
    # 4. 메인 이벤트
    game_start_time = pygame.time.get_ticks() #현재 시간 저장
    while not exit:
        # 4-1. FPS 설정
        clock.tick(60)
        # 4-2. 각종 입력 감지
        ...
        if game_over == False:
            total_time = now_time - game_start_time #시간 기록
        # 새 생성
        ...
        if game_over == True:
            ...
            finalp_img = finalp_font.render(f"Best Record : {best_score/1000}", True, white)
            finalp_size = finalp_img.get_size()
            finalp_pos = tup_r((size[0]/2-finalp_size[0]/2, size[1]/3-finalp_size[1]/2))
            finalp2_img = finalp_font.render(f"Record : {total_time/1000}", True, white)
