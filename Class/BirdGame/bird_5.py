...
class bird:
    def __init__(self):
        ...
        self.img = b_list[self.index][0] #시간에 흐름에 따라 새 그림 변화
        self.size = self.img.get_size()
        self.drop_x = random.randrange(100,size[0]-100-self.size[0]) #랜덤하게 어디서 떨어뜨릴지 설정
        self.drop = False
    ...
    def show(self):
        screen.blit(self.img, self.pos)
class dung: #새똥 클래스
    def __init__(self, x, y, move_x): #새똥의 생성위치, 날아가는 속도에 맞춰 떨어져야함(move_x)
        self.pos = x, y
        self.move_x = move_x
        self.move_y = 2 
        self.a_y = 0.3 #가속도
...
while not exit:
    ...
    # 새 움직임
    bb.timer += bb.ani_move #while문에 돌면 timer무조건 증가
    ...
    # 새똥 움직임
    if bb.drop == True:
        dd.move_y += dd.a_y #가속도를 붙여 점점 속도 증가
        dd.pos = (dd.pos[0]+dd.move_x , dd.pos[1]+dd.move_y)  #위치 업데이트       
    # 4-4. 그리기
    screen.fill(white)
    player.show()
    bb.show()
